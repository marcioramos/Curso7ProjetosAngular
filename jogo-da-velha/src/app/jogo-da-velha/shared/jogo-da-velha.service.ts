...
export class JogoDaVelhaService {
  ...
  fimJogo(linha: number, coluna: number, tabuleiro: any, jogador: number): boolean {
    ...
    if (tabuleiro[0][1] === jogador && tabuleiro[1][1] === jogador && tabuleiro[2][0] === jogador) {
      fim = [[0, 2], [1, 1], [2, 0]]
    }
  ...
  cpuJogar(): void {
    ...
    if (jogada.length <= 0) {
      ...
      for (var i = 0; i < this.TAM_TAB; i++) {
        for (var j = 0; j < this.TAM_TAB; j++) {
  ...
  obterJogada(jogador: number): number[] {
    ...
    for (var lin = 0; lin < this.TAM_TAB; lin++) {
      for (var col = 0; col < this.TAM_TAB; col++) {
